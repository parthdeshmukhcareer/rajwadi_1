...
function Checkout() {
  ...
  const { cartState, refreshCart } = useCart();
      ...
      <div style={{ maxWidth: '1200px', margin: '0 auto', padding: '0 20px' }}>
        ...
        <div style={{ display: 'flex', gap: '30px', flexWrap: 'wrap' }}>
          ...
          <div style={{ flex: '1 1 350px', backgroundColor: '#fff', padding: '30px', borderRadius: '8px', boxShadow: '0 4px 15px rgba(0,0,0,0.05)', height: 'fit-content' }}>
            ...
            {isLoadingSummary ? (
              <p style={{ textAlign: 'center', color: '#888', fontSize: '14px' }}>Calculating totals...</p>
            ) : orderSummary ? (
              <>
                ...
                <div style={{ display: 'flex', justifyContent: 'space-between', marginBottom: '15px', color: '#555', fontSize: '14px', borderBottom: '1px dashed #eaeaea', paddingBottom: '15px' }}>
                  <span>Shipping</span>
                  <span>{orderSummary.shipping > 0 ? `₹${(orderSummary.shipping || 0).toFixed(2)}` : 'FREE'}</span>
                </div>
                <div style={{ display: 'flex', justifyContent: 'space-between', marginBottom: '25px', fontWeight: 'bold', fontSize: '18px', color: '#432227' }}>
                  <span>Total</span>
                  <span>₹{(orderSummary.estimatedTotal || orderSummary.grandTotal || 0).toFixed(2)}</span>
                </div>
              </>
            ) : (
              <div style={{ display: 'flex', justifyContent: 'space-between', marginBottom: '25px', fontWeight: 'bold', fontSize: '18px', color: '#432227' }}>
                <span>Total</span>
                <span>₹{cartItems.reduce((acc, item) => acc + ((Number(item.variant?.price) || Number(item.priceAtAdd) || Number(item.price) || 0) * item.quantity), 0).toFixed(2)}</span>
              </div>
            )}
            ...
            <button 
              ...
              style={{ width: '100%', padding: '16px', backgroundColor: cartItems.length === 0 || isProcessing || isLoadingSummary ? '#ccc' : '#432227', color: '#fff', border: 'none', borderRadius: '4px', fontWeight: 'bold', fontSize: '16px', letterSpacing: '1px', textTransform: 'uppercase', cursor: cartItems.length === 0 || isProcessing || isLoadingSummary ? 'not-allowed' : 'pointer', transition: 'background-color 0.3s' }}
            >
