...
export class ProductsRepository {
  async getProducts(params) {
    ...
    if (productIds.length > 0) {
      ...
      allReviewsStats = await db.select({
        productId: reviews.productId,
        averageRating: sql`ROUND(AVG(${reviews.rating}), 1)`.mapWith(Number),
        reviewCount: sql`count(*)`.mapWith(Number)
      }).from(reviews)
        .where(and(inArray(reviews.productId, productIds), eq(reviews.status, 'PUBLISHED')))
